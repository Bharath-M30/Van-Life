export default function Van({name,price,type,imageUrl}) {
    ...
    return (
        <div className="flex flex-col w-full min-[510px]:w-[14.3125rem] rounded">
            <img src={imageUrl} alt={name} className="size-[20rem] min-[510px]:size-[14.3125rem] rounded-md object-cover"/>
            <div className="flex justify-between mt-4">
                <div>
                    <h3 className="font-[600] text-xl">{name}</h3>
                    {type && <p className={`px-4 py-2 ${typeColor} rounded-md text-center text-white mt-4 w-32`}>{type}</p>}
                </div>
                <div>
